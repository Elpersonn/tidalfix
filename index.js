...
app.get('/browse/track/:id', async (req, res) => {
    ...
    if (dbres && dbres.title) {
        if (botRegex.test(req.get('User-Agent'))) {
            res.send(`
            <head>
                <meta property="og:title" content="${dbres.title}" />
                <meta property="og:description" content="${dbres.description}" />
                <meta property="og:image" content="${dbres.image}" />
                <meta property="twitter:card" content="summary_large_image" />
                <link type="application/json+oembed" href="https://elperson.pro/browse/track/${req.params.id}/json" />
            </head>
            `)
        }else {
...
app.get('/browse/artist/:id', async (req, res) => {
    ...
    if (dbres && dbres.name) {
        if (botRegex.test(req.get("User-Agent"))) {
            res.send(`
            <head>
                <meta property="og:title" content="${dbres.name}" />
                <meta property="og:description" content="${dbres.description}" />
                <meta property="og:image" content="${dbres.image}" />
                <meta property="twitter:card" content="summary_large_image" />
                <link type="application/json+oembed" href="https://elperson.pro/browse/artist/${req.params.id}/json" />
            </head>
            `)
        } else {
        ...
        if (botRegex.test(req.get("User-Agent"))) {
            ...
            <head>
                ...
                <meta property="og:description" content="${dbres.description}" />
                    ...
                    <meta property="og:url" content="https://tidal.com/browse/album/${dbres.id}" />
                    <meta property="twitter:card" content="summary_large_image" />
                    <link type="application/json+oembed" href="https://elperson.pro/browse/album/${dbres.id}/json" />
                </head>
                `)
        } else {
